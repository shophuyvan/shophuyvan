...
async function handleLogin(req, env) {
  try {
    ...
    if (!email || !password) {
      return json({ ok: false, error: 'Email và password là bắt buộc' }, { status: 400 }, req);
    }
    ...
    // Simple password check (demo only - use bcrypt in production)
    ...
    return json({ ok: false, error: 'Lỗi đăng nhập' }, { status: 500 }, req);
  }
}
