...
(async function init(){
  ...
  // [BEGIN PROBE] soi nhanh giá sau khi fetch
try {
  console.groupCollapsed('[FE] Price probe (top 10 items)');
  items.slice(0, 10).forEach(p => {
    const calc = pickLowestPrice(p);
    console.log({
      id: p.id,
      name: p.name,
      price_sale: p.price_sale ?? p.sale_price ?? null,
      price: p.price ?? null,
      variants: Array.isArray(p.variants) ? p.variants.map(v => ({
        price_sale: v.price_sale ?? v.sale_price ?? null,
        price: v.price ?? null
      })) : null,
      FE_pick: calc   // { base, original }
    });
  });
  console.groupEnd();
