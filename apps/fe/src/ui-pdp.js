...
function renderFAQ(){
  ...
  rows.forEach(function(it){
    var d = document.createElement('details'); d.className = 'rounded border p-3';
    d.innerHTML = '<summary class="cursor-pointer font-medium">'+ (it.q||it.question||'') +'</summary><div class="mt-2 text-sm text-gray-700">'+ (it.a||it.answer||'') +'</div>';
    box.appendChild(d);
}
function renderReviews(){
  ...
  rows.forEach(function(r){
    ...
    card.innerHTML = '<div class="text-sm text-gray-600">'+(r.user||'Người mua')+' • '+star+'</div><p class="mt-1 text-sm">'+(r.content||'')+'</p>';
