  ...
  function init() {
    ...
    document.querySelectorAll('.tab').forEach(tab => {
      tab.addEventListener('click', () => {
		...
		// ✅ Kích hoạt Wizard khi vào tab autopost
        if (tab.dataset.tab === 'autopost' && window.AutoSyncWizard) {
          // Không gọi init nếu đã có data (tránh reset)
          if (!window.AutoSyncWizard.jobData || !window.AutoSyncWizard.jobData.productId) {
            setTimeout(() => {
              if (typeof AutoSyncWizard.init === 'function') {
                AutoSyncWizard.init();
              }
            }, 300);
          }
      ...
      if (r && r.ok && r.data) {
        ...
        for (const p of r.data) {
            await Admin.req('/admin/fanpages', {
                ...
                body: {
                    page_id: p.id,
                    page_name: p.name, // ✅ Đã sửa: Gửi đúng key 'page_name' khớp với DB
