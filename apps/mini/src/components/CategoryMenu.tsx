...
// Điều hướng danh mục được xử lý bên trong CategoryMenu bằng hook useNavigate
...
const CategoryNode: React.FC<{
...
}> = ({ category, depth = 0, onSelect, onGotoCategory }) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (hasChildren) {
      ...
      if (slug && onGotoCategory) {
        // Delay nhẹ để menu kịp đóng trước khi điều hướng
        setTimeout(() => {
          onGotoCategory(slug);
        }, 80);
      }
...
export default function CategoryMenu() {
  ...
  const gotoCategory = (slug: string) => {
    if (!slug) return;
    const url = `/category?c=${encodeURIComponent(slug)}`;
    console.log('[CATEGORY MENU CLICK]', slug, url);
    try {
      navigateZmp(url);
    } catch (e) {
      console.error('[CATEGORY MENU CLICK ERROR]', e);
    }
  };
