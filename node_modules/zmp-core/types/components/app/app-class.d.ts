import { zmpDOM, zmpDOMArray } from 'zmp-dom';
...
export interface ZMPEventsClass<Events> {
  /** Add event handler */
  on<E extends keyof Events>(event: E, handler: Events[E]): void;
  /** Add event handler that will be removed after it was fired */
  once<E extends keyof Events>(event: E, handler: Events[E]): void;
...
export interface ZMPParameters {
  ...
  el?: string;
  /** App bundle id.. (default io.zmp.testapp) */
  id?: string | 'io.zmp.testapp';
  /** App name. Can be used by other components, e.g. as the default title for Dialog component.. (default ZMP) */
  name?: string;
  ...
  /** By default ZMP will be initialized automatically when you call new ZMP(). If you want to prevent this behavior you can disable it with this option and then initialize it manually with init() when you need it.. (default true) */
  init?: boolean;
  /** If automatic initialization is enabled with init: true parameter then it will be initialized on deviceready event.. (default true) */
  ...
  url?: string;
  /** Enable bouncing in ios */
  enableBouncing?: boolean;
  /** Object with events handlers.. (default {}) */
  on?: {
    [event in keyof ZMPEvents]?: ZMPEvents[event];
  };
}

export interface ZMPPlugin {
  /** Module Name */
  name: string;
  /** Install callback
...
export interface ZMPEvents {
  /** Event will be fired on app initialization. Automatically after new ZMP() or after app.init() if you disabled auto init. */
  init: () => void;
...
interface ZMP extends ZMPClass<ZMPEvents> {
  ...
  el: HTMLElement;
  /** zmpDOM instance with app root element */
  $el: zmpDOMArray;
  ...
  init(): void;
  /** Load module */
  loadModule(module: string | Function | ZMPPlugin): Promise<any>;
  /** Load modules */
  loadModules(modules: any[]): Promise<any>;
}
interface Events extends ZMPEventsClass<ZMPEvents> {}

declare class ZMP implements ZMP {
  constructor(parameters?: ZMPParameters);

  static use(plugin: ZMPPlugin): void;
  static use(plugins: ZMPPlugin[]): void;
