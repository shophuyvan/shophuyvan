...
var Sheet = /*#__PURE__*/function (_Modal) {
  ...
  function Sheet(app, params) {
    ...
    if (!sheet.params.el) {
      $el = (0, _zmpDom.default)(sheet.params.content).filter(function (node) {
        return node.nodeType === 1;
      }).eq(0);
    } else {
      $el = (0, _zmpDom.default)(sheet.params.el).eq(0);
    }
    ...
    if (sheet.params.backdrop && sheet.params.backdropEl) {
      $backdropEl = (0, _zmpDom.default)(sheet.params.backdropEl);
    } else if (sheet.params.backdrop) {
      $backdropEl = sheet.$containerEl.children('.sheet-backdrop');

      if ($backdropEl.length === 0) {
        $backdropEl = (0, _zmpDom.default)('<div class="sheet-backdrop"></div>');
    ...
    function scrollToElementOnOpen() {
      var $scrollEl = (0, _zmpDom.default)(sheet.params.scrollToEl).eq(0);
    ...
    function handleTouchStart(e) {
      ...
      if (sheet.params.swipeHandler && (0, _zmpDom.default)(e.target).closest(sheet.params.swipeHandler).length === 0) {
        return;
      }
      ...
      if (!sheet.params.swipeHandler && e.type === 'touchstart') {
        sheetPageContentEl = (0, _zmpDom.default)(e.target).closest('.page-content')[0];
      }
    }
    ...
    function handleTouchEnd() {
      ...
      if (timeDiff < 300 && diff > 10) {
        if (direction === openDirection && absCurrentTranslate < absSwipeStepTranslate) {
          ...
          if (sheet.push && pushOffset) {
            sheet.$htmlEl[0].style.setProperty('--zmp-sheet-push-scale', pushViewScale(pushOffset));
            $pushViewEl.css('border-radius', '');
          }
        }
        ...
        if (direction === closeDirection && absCurrentTranslate > absSwipeStepTranslate) {
          ...
          if (sheet.params.swipeToClose) {
            ...
            if (sheet.push && pushOffset) {
              sheet.$htmlEl[0].style.removeProperty('--zmp-sheet-push-scale');
    ...
    sheet.setSwipeStep = function setSwipeStep(byResize) {
      ...
      $el[0].style.setProperty('--zmp-sheet-swipe-step', swipeStepTranslate + "px");
    ...
    sheet.on('open', function () {
      ...
      if (sheet.push) {
        pushOffset = parseInt($el.css('--zmp-sheet-push-offset'), 10);
