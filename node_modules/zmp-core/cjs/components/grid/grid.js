...
var Grid = {
  init: function init() {
    ...
    function handleTouchStart(e) {
      ...
      $resizeHandlerEl = (0, _zmpDom.default)(e.target).closest('.resize-handler');
    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
        ...
        if ($prevResizableEl.length) {
          ...
          gapSize = parseFloat($prevResizableEl.css(isRow ? '--zmp-grid-row-gap' : '--zmp-grid-gap'));
        }
        ...
        if ($nextResizableEl.length) {
          ...
          if (!$prevResizableEl.length) {
            ...
            gapSize = parseFloat($nextResizableEl.css(isRow ? '--zmp-grid-row-gap' : '--zmp-grid-gap'));
          }
        }
      }
      ...
      var gapAddSizeCSS = isRow ? itemsInFlow - 1 + " * var(--zmp-grid-row-gap) / " + itemsInFlow : '(var(--zmp-cols-per-row) - 1) * var(--zmp-grid-gap) / var(--zmp-cols-per-row)';
    ...
    (0, _zmpDom.default)(document).on(app.touchEvents.start, '.col > .resize-handler, .row > .resize-handler', handleTouchStart);
