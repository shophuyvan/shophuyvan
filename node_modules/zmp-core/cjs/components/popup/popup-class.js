...
var Popup = /*#__PURE__*/function (_Modal) {
  ...
  function Popup(app, params) {
    ...
    if (!popup.params.el) {
      $el = (0, _zmpDom.default)(popup.params.content).filter(function (node) {
        return node.nodeType === 1;
      }).eq(0);
    } else {
      $el = (0, _zmpDom.default)(popup.params.el).eq(0);
    }
    ...
    if (popup.params.backdrop && popup.params.backdropEl) {
      $backdropEl = (0, _zmpDom.default)(popup.params.backdropEl);
    } else if (popup.params.backdrop) {
      $backdropEl = popup.$containerEl.children('.popup-backdrop');

      if ($backdropEl.length === 0) {
        $backdropEl = (0, _zmpDom.default)('<div class="popup-backdrop"></div>');
    ...
    function handleTouchStart(e) {
      ...
      if (popup.params.swipeHandler && (0, _zmpDom.default)(e.target).closest(popup.params.swipeHandler).length === 0) {
        return;
      }
      ...
      if (!popup.params.swipeHandler && e.type === 'touchstart') {
        pageContentEl = (0, _zmpDom.default)(e.target).closest('.page-content')[0];
      }
    }
        ...
        if ($pushEl.hasClass('popup')) {
          if ($pushEl.hasClass('popup-push')) {
            $pushEl.transition(0).forEach(function (el) {
              el.style.setProperty('transform', "translate3d(0, calc(-1 * " + pushProgress + " * (var(--zmp-popup-push-offset) + 10px)) , 0px) scale(" + scale + ")", 'important');
            });
          } else {
            $pushEl.transition(0).forEach(function (el) {
    ...
    var updatePushOffset = function updatePushOffset() {
      ...
      if (isPush && !wasPush) {
        ...
        popup.$htmlEl[0].style.setProperty('--zmp-popup-push-scale', pushViewScale(pushOffset));
      } else if (!isPush && wasPush) {
        popup.$htmlEl.removeClass('with-modal-popup-push');
        popup.$htmlEl[0].style.removeProperty('--zmp-popup-push-scale');
      }
    };
    ...
    var setPushOffset = function setPushOffset() {
      ...
      if (isPush) {
        pushOffset = parseInt($el.css('--zmp-popup-push-offset'), 10);
