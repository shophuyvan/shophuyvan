...
var Swipeout = {
  init: function init() {
    ...
    function handleTouchStart(e) {
      ...
      $swipeoutEl = (0, _zmpDom.default)(this);
    }
    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
        if ((0, _zmpDom.default)('.list.sortable-opened').length > 0) return;
      ...
      if ($actionsRight.length > 0) {
        ...
        $rightButtons.each(function (buttonEl) {
          var $buttonEl = (0, _zmpDom.default)(buttonEl);

          if (typeof buttonEl.zmpSwipeoutButtonOffset === 'undefined') {
            $buttonEl[0].zmpSwipeoutButtonOffset = buttonEl.offsetLeft;
          }

          buttonOffset = buttonEl.zmpSwipeoutButtonOffset;
      ...
      if ($actionsLeft.length > 0) {
        ...
        $leftButtons.each(function (buttonEl, index) {
          ...
          if (typeof buttonEl.zmpSwipeoutButtonOffset === 'undefined') {
            $buttonEl[0].zmpSwipeoutButtonOffset = actionsLeftWidth - buttonEl.offsetLeft - buttonEl.offsetWidth;
          }
      ...
      if (action === 'open') {
        ...
        if ($buttons) {
          for (i = 0; i < $buttons.length; i += 1) {
            (0, _zmpDom.default)($buttons[i]).transform("translate3d(" + newTranslate + "px,0,0)");
          }
        }

        if (overswipeRight) {
          $actionsRight.find('.swipeout-overswipe').trigger('click', 'zmpOverswipe');
        }

        if (overswipeLeft) {
          $actionsLeft.find('.swipeout-overswipe').trigger('click', 'zmpOverswipe');
        }
      } else {
    ...
    app.on('touchstart', function (e) {
      if (Swipeout.el) {
        ...
        if (!((0, _zmpDom.default)(Swipeout.el).is($targetEl[0]) || $targetEl.parents('.swipeout').is(Swipeout.el) || $targetEl.hasClass('modal-in') || ($targetEl.attr('class') || '').indexOf('-backdrop') > 0 || $targetEl.hasClass('actions-modal') || $targetEl.parents('.actions-modal.modal-in, .dialog.modal-in').length > 0)) {
          app.swipeout.close(Swipeout.el);
        }
      }
    });
    (0, _zmpDom.default)(document).on(app.touchEvents.start, 'li.swipeout', handleTouchStart, passiveListener);
  ...
  open: function open() {
    ...
    var $el = (0, _zmpDom.default)(el).eq(0);
  ...
  delete: function _delete(el, callback) {
    ...
    $el.transitionEnd(function () {
      ...
      if ($el.parents('.virtual-list').length > 0) {
        var virtualList = $el.parents('.virtual-list')[0].zmpVirtualList;
        var virtualIndex = $el[0].zmpVirtualListIndex;
