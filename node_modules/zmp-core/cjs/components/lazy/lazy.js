...
var Lazy = {
  destroy: function destroy(pageEl) {
    var $pageEl = (0, _zmpDom.default)(pageEl).closest('.page');
    if (!$pageEl.length) return;

    if ($pageEl[0].zmpLazyDestroy) {
      $pageEl[0].zmpLazyDestroy();
    }
  },
  create: function create(pageEl) {
    ...
    var $pageEl = (0, _zmpDom.default)(pageEl).closest('.page').eq(0); // Lazy images
    ...
    if (placeholderSrc !== false) {
      $lazyLoadImages.each(function (lazyEl) {
        if ((0, _zmpDom.default)(lazyEl).attr('data-src') && !(0, _zmpDom.default)(lazyEl).attr('src')) (0, _zmpDom.default)(lazyEl).attr('src', placeholderSrc);
      });
    } // load image
    ...
    if (app.params.lazy.observer && support.intersectionObserver) {
      var observer = $pageEl[0].zmpLazyObserver;
      ...
      $lazyLoadImages.each(function (el) {
        if (el.zmpLazyObserverAdded) return;
        el.zmpLazyObserverAdded = true;
        observer.observe(el);
      });

      if (!$pageEl[0].zmpLazyDestroy) {
        $pageEl[0].zmpLazyDestroy = function () {
          observer.disconnect();
          delete $pageEl[0].zmpLazyDestroy;
          delete $pageEl[0].zmpLazyObserver;
        };
      }
    ...
    function detachEvents() {
      $pageEl[0].zmpLazyAttached = false;
      delete $pageEl[0].zmpLazyAttached;
    ...
    if (!$pageEl[0].zmpLazyDestroy) {
      $pageEl[0].zmpLazyDestroy = detachEvents;
    } // Attach events


    if (!$pageEl[0].zmpLazyAttached) {
      attachEvents();
  ...
  loadImage: function loadImage(imageEl, callback) {
    var app = this;
    var window = (0, _ssrWindow.getWindow)();
    var $imageEl = (0, _zmpDom.default)(imageEl);
  ...
  load: function load(pageEl, callback) {
    ...
    $pageEl.find('.lazy').each(function (lazyEl) {
      var $lazyEl = (0, _zmpDom.default)(lazyEl);
