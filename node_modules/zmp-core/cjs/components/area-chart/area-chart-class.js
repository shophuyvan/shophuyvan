...
var AreaChart = /*#__PURE__*/function (_ZMPClass) {
  _inheritsLoose(AreaChart, _ZMPClass);
  ...
  function AreaChart(app, params) {
    ...
    if ($el[0].zmpAreaChart) return $el[0].zmpAreaChart || _assertThisInitialized(_this);
    ...
    $el[0].zmpAreaChart = self; // Install Modules
  ...
  _proto.setTooltip = function setTooltip() {
    ...
    if (!hasVisibleDataSets) {
      if (self.zmpTooltip && self.zmpTooltip.hide) self.zmpTooltip.hide();
      return;
    }

    if (currentIndex !== null && !self.zmpTooltip) {
      self.zmpTooltip = app.tooltip.create({
        trigger: 'manual',
        containerEl: el,
      ...
      if (self.zmpTooltip && self.zmpTooltip.show) {
        self.zmpTooltip.show();
      }

      return;
    }

    if (!self.zmpTooltip || !self.zmpTooltip.hide || !self.zmpTooltip.show) {
      return;
    }

    if (currentIndex !== null) {
      self.zmpTooltip.setText(self.formatTooltip());
      self.zmpTooltip.setTargetEl(svgEl.querySelector("line[data-index=\"" + currentIndex + "\"]"));
      self.zmpTooltip.show();
    } else {
      self.zmpTooltip.hide();
    }
  };
  ...
  _proto.onLegendClick = function onLegendClick(e) {
    var index = parseInt((0, _zmpDom.default)(e.target).closest('.area-chart-legend-item').attr('data-index'), 10);
  ...
  _proto.update = function update(newParams, onlySvg) {
    ...
    var $rendered = (0, _zmpDom.default)(self.render());
  ...
  _proto.destroy = function destroy() {
    ...
    if (self.zmpTooltip && self.zmpTooltip.destroy) {
      self.zmpTooltip.destroy();
    }

    delete self.$el[0].zmpAreaChart;
