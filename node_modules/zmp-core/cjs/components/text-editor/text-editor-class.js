...
var textEditorButtonsMap = {
  // zmp-icon, material-icon, command
  bold: ['format_bold', 'format_bold', 'bold'],
  italic: ['format_italic', 'format_italic', 'italic'],
  underline: ['format_underlined', 'format_underlined', 'underline'],
  strikeThrough: ['strikethrough_s', 'strikethrough_s', 'strikeThrough'],
  orderedList: ['format_list_numbered', 'format_list_numbered', 'insertOrderedList'],
  unorderedList: ['format_list_bulleted', 'format_list_bulleted', 'insertUnorderedList'],
  link: ['link', 'link', 'createLink'],
  image: ['image', 'image', 'insertImage'],
  paragraph: ['<i class="icon">¶</i>', '<i class="icon">¶</i>', 'formatBlock.P'],
  ...
  alignLeft: ['format_align_left', 'format_align_left', 'justifyLeft'],
  alignCenter: ['format_align_center', 'format_align_center', 'justifyCenter'],
  alignRight: ['format_align_right', 'format_align_right', 'justifyRight'],
  alignJustify: ['format_align_justify', 'format_align_justify', 'justifyFull'],
  subscript: ['<i class="icon">A<sub>1</sub></i>', '<i class="icon">A<sub>1</sub></i>', 'subscript'],
  superscript: ['<i class="icon">A<sup>1</sup></i>', '<i class="icon">A<sup>1</sup></i>', 'superscript'],
  indent: ['format_indent_increase', 'format_indent_increase', 'indent'],
  outdent: ['format_indent_increase', 'format_indent_increase', 'outdent']
};

var TextEditor = /*#__PURE__*/function (_ZMPClass) {
  _inheritsLoose(TextEditor, _ZMPClass);
  ...
  function TextEditor(app, params) {
    ...
    if ($el[0].zmpTextEditor) return $el[0].zmpTextEditor || _assertThisInitialized(_this);
    ...
    if (typeof self.params.buttons === 'string') {
      try {
        ...
        throw new Error('ZMP: TextEditor: wrong "buttons" parameter format');
      }
    }

    $el[0].zmpTextEditor = self; // Bind
    ...
    self.attachEvents = function attachEvents() {
      ...
      (0, _zmpDom.default)(document).on('selectionchange', self.onSelectionChange);
    };
    ...
    self.detachEvents = function detachEvents() {
      ...
      (0, _zmpDom.default)(document).off('selectionchange', self.onSelectionChange);
    }; // Install Modules
  ...
  _proto.createLink = function createLink() {
    ...
    if (currentSelection && currentSelection.anchorNode && (0, _zmpDom.default)(currentSelection.anchorNode).parents(self.$el).length) {
      ...
      var $selectedNodes = (0, _zmpDom.default)(selectedNodes);
      ...
      for (var i = 0; i < $selectedNodes.length; i += 1) {
        ...
        if (childNodes) {
          for (var j = 0; j < childNodes.length; j += 1) {
            if ((0, _zmpDom.default)(childNodes[j]).is('a')) {
      ...
      $selectedLinks = $selectedNodes.closest('a').add((0, _zmpDom.default)(selectedNodesLinks));
    }
  ...
  _proto.onSelectionChange = function onSelectionChange() {
    ...
    var selection = window.getSelection();
    var selectionIsInContent = (0, _zmpDom.default)(selection.anchorNode).parents(self.contentEl).length || selection.anchorNode === self.contentEl;
    ...
    if (self.params.mode === 'popover') {
      var selectionIsInPopover = (0, _zmpDom.default)(selection.anchorNode).parents(self.popover.el).length || selection.anchorNode === self.popover.el;
  ...
  _proto.onBlur = function onBlur() {
    ...
    if (self.params.mode === 'popover') {
      var selection = window.getSelection();
      var selectionIsInContent = (0, _zmpDom.default)(selection.anchorNode).parents(self.contentEl).length || selection.anchorNode === self.contentEl;
      var inPopover = document.activeElement && self.popover && (0, _zmpDom.default)(document.activeElement).closest(self.popover.$el).length;
    ...
    if (self.params.mode === 'keyboard-toolbar') {
      ...
      var _selectionIsInContent = (0, _zmpDom.default)(_selection.anchorNode).parents(self.contentEl).length || _selection.anchorNode === self.contentEl;
  ...
  _proto.onButtonClick = function onButtonClick(e) {
    ...
    var $buttonEl = (0, _zmpDom.default)(e.target).closest('button');
    ...
    if (command.indexOf('formatBlock') === 0) {
      ...
      var $anchorNode = (0, _zmpDom.default)(selection.anchorNode);
  ...
  _proto.renderButtons = function renderButtons() {
    ...
    function renderButton(button) {
      var iconClass = self.app.theme === 'md' ? 'material-icons' : 'material-icons';
  ...
  _proto.createKeyboardToolbar = function createKeyboardToolbar() {
    ...
    self.$keyboardToolbarEl = (0, _zmpDom.default)("<div class=\"toolbar toolbar-bottom text-editor-keyboard-toolbar " + (isDark ? 'theme-dark' : '') + "\"><div class=\"toolbar-inner\">" + self.renderButtons() + "</div></div>");
  };
  ...
  _proto.destroy = function destroy() {
    ...
    delete self.$el[0].zmpTextEditor;
