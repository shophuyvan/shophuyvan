...
var CardExpandable = {
  open: function open(cardEl, animate) {
    ...
    var $cardEl = (0, _zmpDom.default)(cardEl).eq(0);
    ...
    if ($cardEl.attr('data-backdrop-el')) {
      $backdropEl = (0, _zmpDom.default)($cardEl.attr('data-backdrop-el'));
    }
    ...
    if (!$backdropEl && cardParams.backdrop) {
      ...
      if (!$backdropEl.length) {
        $backdropEl = (0, _zmpDom.default)('<div class="card-backdrop"></div>');
    ...
    if (cardParams.hideNavbarOnOpen) {
      ...
      if (!$navbarEl.length) {
        if ($pageEl[0].zmpPage) $navbarEl = $pageEl[0].zmpPage.$navbarEl;
      }
    }
    ...
    var $cardSizeEl = (0, _zmpDom.default)(document.createElement('div')).addClass('card-expandable-size');
    ...
    if ($navbarEl && !cardParams.hideStatusbarOnOpen && maxHeight === pageHeight) {
      statusbarHeight = parseInt($navbarEl.css('--zmp-safe-area-top'), 10);
    ...
    if (cardParams.hideNavbarOnOpen && $navbarEl && $navbarEl.length) {
      if ($navbarEl.closest('.navbar-hidden').length) {
        // Was hidden
        $cardEl[0].zmpKeepNavbarOnClose = true;
      } else {
        delete $cardEl[0].zmpKeepNavbarOnClose;
    ...
    if (cardParams.hideToolbarOnOpen && $toolbarEl && $toolbarEl.length) {
      if ($toolbarEl.closest('.toolbar-hidden').length) {
        // Was hidden
        $cardEl[0].zmpKeepToolbarOnClose = true;
      } else {
        delete $cardEl[0].zmpKeepToolbarOnClose;
    ...
    function onTouchStart(e) {
      if (!(0, _zmpDom.default)(e.target).closest($cardEl).length) return;
  ...
  close: function close(cardEl, animate) {
    ...
    if (cardParams.hideNavbarOnOpen) {
      ...
      if ($navbarEl && $navbarEl.length && !$cardEl[0].zmpKeepNavbarOnClose) {
    ...
    if (cardParams.hideToolbarOnOpen) {
      ...
      if ($toolbarEl && $toolbarEl.length && !$cardEl[0].zmpKeepToolbarOnClose) {
...
var _default = {
  ...
  clicks: {
    ...
    '.card-expandable': function toggleExpandableCard($clickedEl, data, e) {
      ...
      if ((0, _zmpDom.default)(e.target).closest('.card-prevent-open, .card-close').length) return;
    ...
    '.card-backdrop-in': function onBackdropClick() {
      ...
      var $openedCardEl = (0, _zmpDom.default)('.card-opened');
