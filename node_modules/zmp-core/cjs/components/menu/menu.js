...
var Menu = {
  open: function open(el) {
    ...
    var app = this;
    if (!el) return;
    var $el = (0, _zmpDom.default)(el).closest('.menu-item-dropdown');
    if (!$el.length) return;
    ...
    if ($menuEl.length) {
      ...
      $menuEl[0].zmpMenuZIndex = originalZIndex;
    }
  ...
  close: function close(el) {
    ...
    var app = this;
    if (!el) return;
    var $el = (0, _zmpDom.default)(el).closest('.menu-item-dropdown-opened');
    if (!$el.length) return;
    var $menuEl = $el.closest('.menu').eq(0);

    if ($menuEl.length) {
      var zIndex = $menuEl[0].zmpMenuZIndex;
...
var _default = {
  ...
  on: {
    click: function click(e) {
      var app = this;
      var openedMenus = (0, _zmpDom.default)('.menu-item-dropdown-opened');
      if (!openedMenus.length) return;
      openedMenus.each(function (el) {
        if (!(0, _zmpDom.default)(e.target).closest('.menu-item-dropdown-opened').length) {
          app.menu.close(el);
        }
      });
    }
  ...
  clicks: {
    '.menu-item-dropdown': function onClick($clickedEl, dataset, e) {
      ...
      if ($clickedEl.hasClass('menu-item-dropdown-opened')) {
        if ((0, _zmpDom.default)(e.target).closest('.menu-dropdown').length) return;
