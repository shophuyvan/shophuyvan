...
function backward(router, el, backwardOptions) {
    ...
    if ($oldPage.prev('.page-previous:not(.stacked)').length > 0 || $oldPage.prev('.page-previous').length === 0) {
      ...
      if (backIndex && router.params.stackPages) {
        $oldPage.prevAll('.page-previous').each(function (pageToRemove) {
          var $pageToRemove = (0, _zmpDom.default)(pageToRemove);
          var $navbarToRemove;

          if (dynamicNavbar) {
            // $navbarToRemove = $oldNavbarEl.prevAll('.navbar-previous').eq(index);
            $navbarToRemove = (0, _zmpDom.default)(app.navbar.getElByPage($pageToRemove));
          }
  ...
  function insertPage() {
    ...
    if ($newPage.next($oldPage).length === 0) {
      if (!newPageInDom && zmpComponent) {
        zmpComponent.mount(function (componentEl) {
          (0, _zmpDom.default)(componentEl).insertBefore($oldPage);
        });
      } else {
  ...
  if (options.preload) {
    ...
    if (isMaster) {
      ...
      if (dynamicNavbar) {
        (0, _zmpDom.default)(app.navbar.getElByPage($newPage)).removeClass('navbar-master-stacked');
